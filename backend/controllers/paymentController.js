    ...
    if (session.payment_status === 'paid') {
      ...
      if (!booking) {
        ...
        const notification = await Notification.create({
          ...
          type: 'BOOKING',
          message: `New Booking! Your item has been rented for $${session.amount_total / 100}.`,
          bookingId: booking._id,
        });

        const io = req.app.get('socketio');
        if (io) {
          io.to(session.metadata.ownerId).emit('notification received', notification);
        }
      }
...
exports.stripeWebhook = async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (!existingBooking) {
      ...
      const io = req.app.get('socketio');
      if (io) io.to(session.metadata.ownerId).emit('notification received', notification);
    }
  }
